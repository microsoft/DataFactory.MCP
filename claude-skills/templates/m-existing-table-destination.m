// Template: Existing table destination (UI-created dataflows only)
// For API-created dataflows, use m-new-table-destination.m even for existing tables
// Replace: QUERY_NAME, WORKSPACE_ID, LAKEHOUSE_ID
// After saving: add_connection_to_dataflow → execute_query to validate → refresh_dataflow_background

section Section1;

[DataDestinations = {[
  Definition = [Kind = "Reference", QueryName = "QUERY_NAME_DataDestination", IsNewTarget = false],
  Settings = [
    Kind = "Manual",
    AllowCreation = false,
    DynamicSchema = false,
