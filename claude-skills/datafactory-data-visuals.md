...
other `Parent` resolves to a `Name`, and relationships are acyclic. The root can
be any PartType. Prefer a `Card` for chart titles, but a chart can be a root or
container child and use its own `ChartTitle`. Format KPI values as text.
...
4. Select required columns, filter invalid values, aggregate chart data to visual
   grain, sort explicitly, and cap categories before building the flat `#table`.
...
6. For the Advanced Editor, return a complete single-query expression. For MCP
   ...
   Ask before changing a dataflow. Use `save_dataflow_definition` with
...
cross-filtering. They render only in the authoring canvas, not refresh output,
...
need not represent equal elapsed time. Structural validation cannot prove that
